...
export const getEmailTemplates = async (companyUid?: string): Promise<EmailTemplate[]> => {
...
export const updateEmailTemplate = async (uid: string, data: UpdateEmailTemplateDto): Promise<EmailTemplate> => {
...
export const previewEmailTemplate = async (uid: string, data?: PreviewEmailTemplateDto): Promise<PreviewEmailTemplateResponse> => {
  const response = await axiosInstance.post(`/email-templates/${uid}/preview`, data || {});
  return response.data;
};
