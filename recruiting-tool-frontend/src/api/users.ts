...
export const usersApi = {
	...
	getResumeDownloadUrl: async (): Promise<{downloadUrl: string; fileName: string}> => {
		const response = await axiosInstance.get<{downloadUrl: string; fileName: string}>('/users/resume/download');
		return response.data;
	},

	deleteResume: async (): Promise<{message: string}> => {
		const response = await axiosInstance.delete<{message: string}>('/users/resume');
		return response.data;
	},
};
