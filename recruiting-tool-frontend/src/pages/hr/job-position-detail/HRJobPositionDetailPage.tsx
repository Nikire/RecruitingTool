	...
	if (error || !jobPositionData) {
		return (
			<Box sx={{p: {xs: 2, sm: 3, md: 4}}}>
				<Typography color="error">{t('job_position_detail.error_loading')}</Typography>
				<Button
		...
		<Box sx={{p: {xs: 2, sm: 3, md: 4}}}>
			...
			<Paper sx={{p: {xs: 2, sm: 3}, mb: {xs: 2, sm: 3}}}>
				...
				>
					...
					<Box sx={{display: 'flex', gap: 2, alignItems: 'center', width: {xs: '100%', sm: 'auto'}, flexWrap: 'wrap'}}>
			...
			<Box sx={{mb: {xs: 2, sm: 3}, display: 'flex', gap: 1, flexWrap: 'wrap'}}>
				{statusOptions.map((status) => {
					const count =
						status === 'ALL'
							? jobPosition.hiringProcesses?.length || 0
							: jobPosition.hiringProcesses?.filter((p) => p.status === status).length ||
							  0;
			...
			<Typography
				variant="h5"
				sx={{mb: {xs: 1.5, sm: 2}, mt: {xs: 3, sm: 5}, fontSize: {xs: '1.25rem', sm: '1.5rem'}}}
			>
			...
			<Typography
				variant="h5"
				sx={{mb: {xs: 1.5, sm: 2}, mt: {xs: 3, sm: 4}, fontSize: {xs: '1.25rem', sm: '1.5rem'}}}
			>
