...
const TeamManagementPage: React.FC = () => {
  ...
  // Fetch data
  const { data: members, isLoading: isLoadingMembers } = useCompanyMembers(companyUid);
  const { data: invitations, isLoading: isLoadingInvitations } = useCompanyInvitations(
    companyUid,
  );
          ...
          <Tab
            ...
            label={
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Chip label={pendingInvitations.length} size="small" color="primary" />
              </Box>
            }
          />
          <Tab
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {t('team.invited_by', { name: invitation.invitedByName })} •{' '}
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" sx={{ mt: 1 }}>
                          {t('team.requested_role', {
                            role: t(`roles.${request.requestedRole.toLowerCase()}`),
                          })}
                        </Typography>
                        {request.message && (
                          <Typography variant="caption" color="text.secondary">
