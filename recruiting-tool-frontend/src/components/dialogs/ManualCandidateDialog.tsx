			...
			<form onSubmit={handleSubmit(onSubmit)} aria-label={t('aria.create_dialog', {entity: t('candidates.title').toLowerCase()})}>
				<DialogContent>
					...
					<TextField
						...
						error={!!errors.phone}
						helperText={errors.phone?.message || t('manual_candidate.phone_helper')}
						inputProps={{
					...
					<Controller
						...
						render={({field}) => (
							...
							>
								...
								{jobPositions?.filter(jp => jp.status === 'OPEN').map((position) => (
					...
					<TextField
						...
						error={!!errors.notes}
						helperText={errors.notes?.message || t('manual_candidate.notes_helper')}
				...
				<DialogActions>
					...
					<Button
						...
						variant="contained"
						disabled={isPending}
						startIcon={isPending ? <CircularProgress size={20} color="inherit" aria-label={t('aria.loading')} /> : <PersonAddIcon />}
						aria-label={isPending ? t('common.creating') : t('manual_candidate.create_button')}
					>
						{isPending ? t('common.creating') : t('manual_candidate.create_button')}
					</Button>
				</DialogActions>
			</form>
		</Dialog>
