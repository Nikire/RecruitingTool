...
const StageProgressionDialog: React.FC<StageProgressionDialogProps> = ({
  ...
  const nextStageIndex = currentStage
    ? stages.findIndex((s) => s.uid === currentStageUid) + 1
    : -1;
      ...
      <DialogContent sx={{ pt: 2 }}>
        {progressStageMutation.error || moveToStageMutation.error ? (
          <Alert severity="error" sx={{ mb: 2 }}>
            {progressStageMutation.error?.message ||
              moveToStageMutation.error?.message}
          </Alert>
        ) : null}
        ...
        <Box sx={{ mb: 3 }}>
          ...
          {!hasNextStage && currentStage?.status === "CURRENT" && (
            <Alert severity="info">{t("progress.final_stage_notice")}</Alert>
          )}
        </Box>
