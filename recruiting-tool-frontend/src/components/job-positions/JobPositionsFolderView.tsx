import {Box, Typography, CircularProgress, Card, CardContent, CardActionArea, Chip, Divider, Paper} from '@mui/material';
...
const JobPositionsFolderView: React.FC<JobPositionsFolderViewProps> = ({hiringProcesses}) => {
	...
	const jobPositionStats = jobPositions?.map(jp => {
		const processCount = hiringProcesses?.filter(p => p.jobPosition?.uid === jp.uid).length || 0;
		const statusCounts = {
			OPEN: hiringProcesses?.filter(p => p.jobPosition?.uid === jp.uid && p.status === 'OPEN').length || 0,
			IN_PROGRESS: hiringProcesses?.filter(p => p.jobPosition?.uid === jp.uid && p.status === 'IN_PROGRESS').length || 0,
			CLOSED: hiringProcesses?.filter(p => p.jobPosition?.uid === jp.uid && p.status === 'CLOSED').length || 0,
		};
