...
export const useAssignRole = (companyUid: string) => {
	...
	return useMutation({
		mutationFn: (data: AssignRoleDto) => companyRolesApi.assignRole(companyUid, data),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: [COMPANY_ROLES_KEY, companyUid] });
...
export const useRemoveUserFromCompany = (companyUid: string) => {
	...
	return useMutation({
		mutationFn: (userUid: string) => companyRolesApi.removeUserFromCompany(companyUid, userUid),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: [COMPANY_ROLES_KEY, companyUid] });
