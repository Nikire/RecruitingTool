...
test.describe('Email Templates Management', () => {
  ...
  test('should display email templates list', async ({ page }) => {
    ...
    const templates = page.locator('.MuiCard-root, [role="article"], .MuiPaper-root');
    const count = await templates.count();

    const hasTemplates = count > 0;
    const hasEmptyState = await page.locator('text=/no.*template|sin.*plantilla/i').count() > 0;
  ...
  test('should display template information', async ({ page }) => {
    ...
    const templateInfo = page.locator('text=/template|subject|asunto|plantilla/i').first();
  ...
  test('should open edit template dialog', async ({ page }) => {
    ...
    if (await editButton.count() > 0) {
      ...
      const editor = dialog.locator('textarea, [contenteditable="true"], .tiptap, .quill-editor').first();
  ...
  test('should display template categories or types', async ({ page }) => {
    ...
    const categories = page.locator('text=/application|interview|rejection|aceptación|entrevista|rechazo/i');
  ...
  test('should show template preview', async ({ page }) => {
    ...
    if (await previewButton.count() > 0) {
      ...
      const preview = page.locator('.MuiDialog-paper, [class*="preview"]').first();
  ...
  test('should display template status or language', async ({ page }) => {
    ...
    const statusOrLang = page.locator('text=/active|inactive|english|spanish|activo|inactivo|inglés|español/i, .MuiChip-root');
  ...
  test('should show variable placeholders documentation', async ({ page }) => {
    ...
    const variableInfo = page.locator('text=/variable|placeholder|{{.*}}|candidateName|positionTitle/i').first();
