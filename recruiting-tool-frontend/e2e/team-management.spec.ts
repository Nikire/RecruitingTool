...
test.describe('Team Management', () => {
  ...
  test('should display team members list', async ({ page }) => {
    ...
    const hasEmptyState = await page.locator('text=/no.*members|no.*team|sin.*miembros/i').count() > 0;
  ...
  test('should display role badges for team members', async ({ page }) => {
    ...
    const roleIndicators = page.locator('.MuiChip-root, [class*="badge"], [class*="role"]');
  ...
  test('should show team member actions', async ({ page }) => {
    ...
    const actionButtons = page.locator('button[aria-label*="edit"], button[aria-label*="delete"], button[aria-label*="manage"]').first();
  ...
  test('should display pending invitations', async ({ page }) => {
    ...
    const pendingSection = page.locator('text=/pending.*invitation|invitation.*pending|invitaciones.*pendientes/i').first();
  ...
  test('should show team member status', async ({ page }) => {
    ...
    const statusIndicators = page.locator('text=/active|inactive|pending|activo|inactivo|pendiente/i, .MuiChip-root');
