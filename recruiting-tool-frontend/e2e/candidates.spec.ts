...
test.describe("Candidates Management", () => {
  ...
  test("should display candidates list page", async ({ page }) => {
    ...
    const title = page
      .locator("h4")
      .filter({ hasText: /candidates|candidatos/i })
      .first();
  ...
  test("should display candidate data table or cards", async ({ page }) => {
    ...
    const dataContainer = page
      .locator('[role="grid"], table, .MuiDataGrid-root, .MuiCard-root')
      .first();
  ...
  test("should have create candidate button", async ({ page }) => {
    ...
    const createButton = page
      .locator("button")
      .filter({ hasText: /add|create|new|agregar|crear/i })
      .first();
    await expect(createButton).toBeVisible({ timeout: 5000 });
  });

  test("should open create candidate dialog when clicking create button", async ({
    page,
  }) => {
    // Click create button
    const createButton = page
    ...
    if (await menu.isVisible()) {
      ...
      await page
        .waitForSelector('[role="menu"]', { state: "hidden", timeout: 3000 })
        .catch(() => {});
  ...
  test("should show candidate data rows", async ({ page }) => {
    ...
    const hasEmptyState =
      (await page
        .locator("text=/no.*candidates|no.*data|sin.*candidatos/i")
        .count()) > 0;
  ...
  test("should navigate to candidate details when clicking view", async ({
    page,
  }) => {
    await waitForTableData(page);
    ...
    const viewButton = page
      .locator('button[aria-label*="view"], button[aria-label*="View"]')
      .first();
  ...
  test("should have pagination controls", async ({ page }) => {
    ...
    const pagination = page.locator(
      '[aria-label*="pagination"], .MuiTablePagination-root, [class*="pagination"]',
    );
  ...
  test("should sort by clicking column headers", async ({ page }) => {
    ...
    if ((await sortableHeader.count()) > 0) {
      ...
      await page.locator('[role="row"]').nth(1).textContent();
...
test.describe("Candidate Form Validation", () => {
  ...
  test("should show validation errors for empty required fields", async ({
    ...
    const submitButton = dialog
      ...
      .filter({ hasText: /save|create|submit|guardar|crear/i })
      .first();
    ...
    if ((await submitButton.count()) > 0) {
      ...
      const muiErrors = await page
        .locator('.Mui-error, [class*="error"]')
        .count();
      const requiredTexts = await page
        .locator("text=/required|requerido/i")
        .count();
  ...
  test("should validate email format", async ({ page }) => {
    ...
    if ((await emailInput.count()) > 0) {
      ...
      const hasEmailError =
        (await page
          .locator("text=/invalid.*email|email.*invalid|correo.*inválido/i")
          .count()) > 0;
      const inputHasError = await emailInput.evaluate(
        (el) =>
