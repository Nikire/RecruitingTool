...
test.describe("Calendar Settings", () => {
  ...
  test("should display Google Calendar integration section", async ({
    page,
  }) => {
    await waitForLoadingComplete(page);

    // Look for Google Calendar mentions
    const googleCalendar = page
      .locator(
        "text=/google.*calendar|calendar.*integration|integración.*calendario/i",
      )
      .first();
  ...
  test("should display calendar connection status", async ({ page }) => {
    ...
    const statusIndicator = page
      .locator(
        "text=/connected|disconnected|authorized|not.*connected|conectado|no.*conectado/i, .MuiChip-root",
      )
      .first();
  ...
  test("should display calendar availability settings", async ({ page }) => {
    ...
    const availabilitySettings = page
      .locator("text=/availability|working.*hours|disponibilidad|horario/i")
      .first();
  ...
  test("should have time zone selector", async ({ page }) => {
    ...
    const timeZoneSelector = page
      .locator('[role="combobox"], select, input')
      .filter({
  ...
  test("should display interview buffer time settings", async ({ page }) => {
    ...
    const bufferSettings = page
      .locator("text=/buffer|duration|interview.*time|tiempo.*entrevista/i")
      .first();
  ...
  test("should display calendar permissions information", async ({ page }) => {
    ...
    const permissionsInfo = page
      .locator("text=/permission|scope|access|permiso|acceso/i")
      .first();
  ...
  test("should show calendar event types configuration", async ({ page }) => {
    ...
    const eventTypes = page
      .locator(
        "text=/event.*type|interview.*type|tipo.*evento|tipo.*entrevista/i",
      )
      .first();
