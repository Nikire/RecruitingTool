...
test.describe('Onboarding Wizard', () => {
  ...
  test('should display onboarding wizard', async ({ page }) => {
    ...
    const isRedirected = url.includes('/hr') || url.includes('/admin') || url.includes('/dashboard');
  ...
  test('should display wizard steps indicator', async ({ page }) => {
    ...
    const stepper = page.locator('.MuiStepper-root, [class*="stepper"], [class*="wizard"]');
  ...
  test('should navigate between steps', async ({ page }) => {
    ...
    if (await nextButton.count() > 0) {
      // Get current step
      const currentStep = await page.locator('.MuiStep-root.Mui-active, [class*="active"]').count();
  ...
  test('should have skip option', async ({ page }) => {
    ...
    const skipButton = page.locator('button, a').filter({
...
test.describe('HR Onboarding Wizard', () => {
  ...
  test('should display HR-specific onboarding steps', async ({ page }) => {
    ...
    const hrContent = page.locator('text=/company|position|job|team|empresa|puesto|equipo/i').first();
