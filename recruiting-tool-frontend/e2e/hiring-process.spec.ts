...
test.describe("Applications Management (Hiring Processes)", () => {
  ...
  test("should display application list or cards", async ({ page }) => {
    ...
    const hasEmptyState =
      (await page
        .locator("text=/no.*application|no.*data|sin.*solicitud/i")
        .count()) > 0;
  ...
  test("should display relevant columns/information", async ({ page }) => {
    ...
    const statusInfo = page
      .locator("text=/status|estado|stage|etapa/i")
      .first();
  ...
  test("should have action buttons for applications", async ({ page }) => {
    ...
    const actionButtons = page
      .locator("button, a")
      .filter({ hasText: /view|ver|details|detalles/i })
      .first();
  ...
  test("should open application details dialog", async ({ page }) => {
    ...
    if ((await viewButton.count()) > 0) {
      ...
      const hasNavigated =
        ...
        page.url().includes("/hiring-process");
  ...
  test("should display application stages/pipeline", async ({ page }) => {
    ...
    const stageIndicators = page.locator(
      '.MuiStepper-root, [class*="timeline"], [class*="stage"], .MuiChip-root',
    );
  ...
  test("should update application status", async ({ page }) => {
    ...
    if ((await firstApplication.count()) > 0) {
      ...
      const statusButton = page
        ...
        .filter({ hasText: /status|update.*status|cambiar.*estado/i })
        .first();
  ...
  test("should move candidate between stages", async ({ page }) => {
    ...
    if ((await firstApplication.count()) > 0) {
      ...
      const moveButton = page
        .locator("button")
        .filter({ hasText: /move|advance|next|siguiente|avanzar/i })
        .first();
  ...
  test("should add notes to application", async ({ page }) => {
    ...
    if ((await firstApplication.count()) > 0) {
      ...
      const addNoteButton = page
        .locator("button")
        .filter({ hasText: /add.*note|note|comment|nota|comentario/i })
        .first();
      ...
      if ((await addNoteButton.count()) > 0) {
        ...
        if ((await noteInput.count()) > 0) {
          ...
          const submitButton = page
            .locator("button")
            .filter({ hasText: /save|submit|add|guardar|agregar/i })
            .first();
...
test.describe("HR Dashboard", () => {
  ...
  test("should display HR dashboard", async ({ page }) => {
    ...
    if (isOnLogin) {
      console.log(
        "Warning: Redirected to login - possible race condition in user state loading",
      );
    }
  ...
  test("should navigate from dashboard to other pages", async ({ page }) => {
    ...
    const navLinks = page.locator(
      'nav a[href*="/hr/"]:not([href*="dashboard"])',
    );
