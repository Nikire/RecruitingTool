...
import { login, logout, TEST_USERS } from "./fixtures/test-utils";
...
test.describe("Authentication Flow", () => {
  ...
  test("should show error with invalid credentials", async ({ page }) => {
    ...
    const errorMessage = page
      .locator("text=/invalid|incorrect|wrong|failed/i")
      .first();
  ...
  test("should redirect to login when accessing protected route without auth", async ({
    ...
    const hasAccessDenied =
      (await page
        .locator("text=/access.*denied|not.*authorized|no.*permiso/i")
        .count()) > 0;
    const hasLoginForm =
      (await page.locator('form input[type="password"]').count()) > 0;

    expect(
      url.includes("/login") || hasAccessDenied || hasLoginForm,
    ).toBeTruthy();
  });
  ...
  test("should handle different user roles correctly", async ({ page }) => {
    ...
    const authIndicator = page
      .locator('.MuiAvatar-root, [role="banner"]')
      .first();
