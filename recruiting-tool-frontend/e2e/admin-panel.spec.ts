...
test.describe('Admin Panel - Companies Management', () => {
  ...
  test('should display companies list', async ({ page }) => {
    ...
    const hasEmptyState = await page.locator('text=/no.*companies|no.*data|sin.*empresas/i').count() > 0;
  ...
  test('should display company information columns', async ({ page }) => {
    ...
    const statusColumn = page.locator('[role="columnheader"], th').filter({ hasText: /status|estado/i }).first();
  ...
  test('should show company action buttons', async ({ page }) => {
    ...
    const actionButtons = page.locator('button[aria-label*="edit"], button[aria-label*="view"], button[aria-label*="delete"]').first();
  ...
  test('should display subscription status for companies', async ({ page }) => {
    ...
    const subscriptionStatus = page.locator('text=/subscription|trial|active|expired|suscripción|activo|expirado/i, .MuiChip-root');
...
test.describe('Admin Panel - User Management', () => {
  ...
  test('should display users list', async ({ page }) => {
    ...
    const hasData = count > 0;
    const hasEmptyState = await page.locator('text=/no.*users|no.*data|sin.*usuarios/i').count() > 0;
  ...
  test('should display user information columns', async ({ page }) => {
    ...
    const roleColumn = page.locator('[role="columnheader"], th').filter({ hasText: /role|rol/i }).first();
  ...
  test('should filter users by role', async ({ page }) => {
    ...
    const roleFilter = page.locator('button, [role="combobox"]').filter({
  ...
  test('should display user status', async ({ page }) => {
    ...
    const statusIndicators = page.locator('text=/active|inactive|activo|inactivo/i, .MuiChip-root');
  ...
  test('should have user action buttons', async ({ page }) => {
    ...
    const actionButtons = page.locator('button[aria-label*="edit"], button[aria-label*="delete"]').first();
...
test.describe('Admin Panel - System Settings', () => {
  ...
  test('should display general settings', async ({ page }) => {
    ...
    const settingsFields = page.locator('input, select, textarea, [role="combobox"]');
...
test.describe('Admin Panel - Deleted Records', () => {
  ...
  test('should display deleted items list', async ({ page }) => {
    ...
    const hasData = count > 0;
    const hasEmptyState = await page.locator('text=/no.*deleted|no.*records|sin.*eliminados/i').count() > 0;
  ...
  test('should filter deleted records by type', async ({ page }) => {
    ...
    const typeFilter = page.locator('button, [role="combobox"]').filter({
  ...
  test('should display deletion date and user', async ({ page }) => {
    ...
    const deletionInfo = page.locator('text=/deleted.*by|deleted.*on|eliminado.*por|fecha.*eliminación/i').first();
