...
### 🧑‍💼 HR candidates & hiring processes (21 verified findings)
- Notification deep links: hiring-process highlights auto-expand their group and scroll into view; candidate notifications link to the candidate detail page instead of an ignored query param
- Plurals fixed project-wide: 15 legacy `_plural` keys converted to i18next v4 `_one`/`_other`, so "5 processes", "3 stages", "2 applicants" etc. render correctly (they previously showed the singular form)
- Search/status changes reset pagination on the candidates and hiring-process pages; empty states distinguish "no results for this filter" (with Clear) from "nothing yet" (with Create)
- Grouped hiring-process list: AI rankings only requested for expanded groups; header wraps on phones; stage chips get tooltips; keyboard semantics corrected
...
### 🧩 Shared components (19 verified findings + 12 adjacent)
- "Add candidate manually" now keeps its promise: initial notes are saved as a candidate note and a selected open position starts the hiring process
- Every DataGrid renders its built-in UI (pagination, column menu, filters, "No rows") in the selected language; grid date cells and their tooltips localised; "items per page" label fits in Spanish
- Dialogs: delete confirmations disable while pending (applications, manage stages); stage progression resets its selection, uses theme colours in dark mode and handles request failures; reorder save watches its own pending flag; salary range rejects min > max; stage-type chips and stage statuses translated; application dates localised; long applicant fields wrap; company picker in Update User instead of a free-text UID; email format validated before sending a change code; date pickers follow the app locale; optional Source Details no longer says "is required"; English plural "s" removed from a translated label
- Forms & common: screening question text no longer ellipsised as a field label; multiple-choice options are kept without pressing Enter; question cards get labelled, confirmed actions; status chip edit affordance works; markdown editor trims to max length instead of rejecting the change; debounced search no longer drops characters; advanced search debounced; status filter chips translated by default; feedback hide/restore controls keyboard-accessible; "What's new" closes immediately

### 🔌 Data layer: services, auth and cache (19 verified findings)
- Three services used a bare axios client instead of the configured instance, so they never sent the auth header nor unwrapped the response envelope. Connection requests crashed the Team Management page, the recycle bin was permanently empty, and notification preferences requested `undefined/...`. All three now use the shared instance
- Logout clears the whole query cache (the previous account's data survived an in-tab account switch) and calls the logout endpoint so the refresh token is revoked server-side
- A silent token wipe on a public route now updates the UI instead of leaving a signed-out user looking signed in
- Cache invalidation fixed: prospect mutations refresh the CRM table, accepting an application refreshes candidates and hiring processes, quota-consuming mutations refresh the quota banner, soft deletes refresh the recycle bin, disconnecting Google Calendar clears its settings
