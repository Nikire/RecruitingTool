import { Injectable, Logger, ConflictException, NotFoundException, HttpException, InternalServerErrorException, BadRequestException } from '@nestjs/common';
...
export class UsersService {
  ...
  async createInternal(createUserDto: CreateUserInternalDto): Promise<UserResponseDto> {
    try {
      ...
      throw new InternalServerErrorException(`Failed to create internal: ${error.message}`);
    }
  }
      ...
      if (updateUserDto.profilePicture !== undefined && existingUser.profilePicture) {
        ...
        if (updateUserDto.profilePicture !== existingUser.profilePicture) {
          try {
            ...
            this.logger.error(`Failed to delete old profile picture ${existingUser.profilePicture}:`, error.message);
          }
        }
      }
  ...
  async findByEmail(email: string): Promise<UserWithPasswordResponseDto> | null {
    try {
      ...
      throw new InternalServerErrorException(`Failed to find by email: ${error.message}`);
    }
  }
  ...
  async deactivateUser(uid: string, adminUserId: number): Promise<UserResponseDto> {
    try {
      ...
      throw new InternalServerErrorException(`Failed to deactivate user: ${error.message}`);
    }
  }
  ...
  async reactivateUser(uid: string, adminUserId: number): Promise<UserResponseDto> {
    try {
      ...
      throw new InternalServerErrorException(`Failed to reactivate user: ${error.message}`);
    }
  }
