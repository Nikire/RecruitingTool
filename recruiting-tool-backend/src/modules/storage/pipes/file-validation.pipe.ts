...
export class FileValidationPipe implements PipeTransform {
	...
	async transform(file: Express.Multer.File): Promise<Express.Multer.File> {
		...
		this.logger.log(
			`Validating ${this.validationType} file: ${file.originalname} (${file.mimetype}, ${file.size} bytes)`,
		);

		// Perform validation based on type
		const validationResult =
			this.validationType === 'document'
				? await FileValidator.validateDocument(file)
				: await FileValidator.validateImage(file);

		if (!validationResult.isValid) {
			this.logger.warn(
				`File validation failed for ${file.originalname}: ${validationResult.error}`,
			);

			// Log security violations
			if (
				validationResult.error?.includes('does not match') ||
				validationResult.error?.includes('Executable') ||
				validationResult.error?.includes('Invalid characters')
			) {
				this.logger.error(
					`SECURITY VIOLATION: File upload blocked - ${file.originalname} - Reason: ${validationResult.error}`,
				);
			}
