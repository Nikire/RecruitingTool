	...
	private static async validateFile(
		...
		try {
			...
			if (!magicNumberValid.isValid) {
				this.logger.warn(
					`Magic number validation failed for file: ${file.originalname} (MIME: ${mimeType})`,
				);
				return {
					isValid: false,
			...
			this.logger.log(
				`File validation successful: ${file.originalname} (${mimeType}, ${file.size} bytes)`,
			);
	...
	private static performSecurityChecks(file: Express.Multer.File): FileValidationResult {
		...
		for (const pattern of suspiciousPatterns) {
			if (pattern.test(file.originalname)) {
				this.logger.warn(
					`Suspicious file extension detected: ${file.originalname} (blocked executable type)`,
				);
				return {
					isValid: false,
